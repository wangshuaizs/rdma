...
void rc_client_loop(const char *host, const char *port, void *context)
{
  ...
  TEST_NZ(rdma_set_option(conn, RDMA_OPTION_ID, RDMA_OPTION_ID_TOS, &optval, sizeof(optval)));
...
void rc_server_loop(const char *port)
{
  ...
  TEST_NZ(rdma_set_option(listener, RDMA_OPTION_ID, RDMA_OPTION_ID_TOS,&optval, sizeof(optval)));
